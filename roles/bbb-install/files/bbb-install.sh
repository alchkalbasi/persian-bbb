...
main() {
  ...
  # Docker and Docker Compose are NOT installed by this script anymore, they are
  # expected to be provisioned externally (e.g. by Ansible).  DOCKER_COMPOSE_BIN
  # may be exported before running this script to force a specific command,
  # e.g. DOCKER_COMPOSE_BIN="docker-compose" or DOCKER_COMPOSE_BIN="docker compose".
  ...
  if [ "$DISTRO" == "jammy" ]; then
    ...
    touch /root/.rnd
    check_docker		                     # needed for bbb-libreoffice-docker (docker must already be installed)
    need_pkg ruby
...
install_greenlight_v3(){
  ...
  if [ -n "$INSTALL_KC" ]; then
      ...
      docker_compose -f $GL3_DIR/docker-compose.yml up -d postgres && say "started postgres"
      wait_postgres_start
      docker_compose -f $GL3_DIR/docker-compose.yml exec -T postgres psql -U postgres -c 'CREATE DATABASE keycloakdb;'
  fi
  ...
  if ! grep -q 'keycloak:' $GL3_DIR/docker-compose.yml; then
    ...
    if [ -n "$INSTALL_KC" ]; then
      ...
      docker_compose -f $GL3_DIR/docker-compose.yml down
...
HERE
  ...
  docker_compose -f $GL3_DIR/docker-compose.yml pull
  ...
  docker_compose -f $GL3_DIR/docker-compose.yml up -d
  sleep 5
...
install_lti(){
  ...
  docker_compose -f $LTI_DIR/docker-compose.yml pull
  ...
  if check_container_running broker; then
    ...
    docker_compose -f $LTI_DIR/docker-compose.yml down
  fi

  say "starting BBB LTI framework services..."
  docker_compose -f $LTI_DIR/docker-compose.yml up -d
  ...
  if ! docker_compose -f $LTI_DIR/docker-compose.yml exec -T broker bundle exec rake db:keys:update["$LTI_KEY","$LTI_SECRET"] \
    2> /dev/null 1>&2; then
    docker_compose -f $LTI_DIR/docker-compose.yml exec -T broker bundle exec rake db:keys:add["$LTI_KEY","$LTI_SECRET"] \
...
wait_lti_broker_start() {
  ...
  docker_compose -f $LTI_DIR/docker-compose.yml up -d broker || err "failed to register LTI framework apps due to LTI broker failling to start - retry to resolve"

  local tries=0
  while ! docker_compose -f $LTI_DIR/docker-compose.yml exec -T broker bundle exec rake db:version 2> /dev/null 1>&2; do
    echo -n .
    sleep 3
    if (( ++tries == 3 )); then
...
wait_postgres_start() {
  ...
  docker_compose -f $GL3_DIR/docker-compose.yml up -d postgres || err "failed to start Postgres service - retry to resolve"

  local tries=0
  while ! docker_compose -f $GL3_DIR/docker-compose.yml exec -T postgres pg_isready 2> /dev/null 1>&2; do
    echo -n .
    sleep 3
    if (( ++tries == 3 )); then
...
register_lti_tool() {
 # Preparing and checking the environment.
  ...
  if ! docker_compose -f $LTI_DIR/docker-compose.yml exec -T broker bundle exec rake db:apps:show["$APP_NAME"] \
    2> /dev/null 1>&2; then
    docker_compose -f $LTI_DIR/docker-compose.yml exec -T broker bundle exec rake db:apps:add["$APP_NAME","$CALLBACK_URI","$OAUTH_KEY","$OAUTH_SECRET"] \
      2> /dev/null 1>&2 && say "$LOG_NAME was successfully registered."
  else
    docker_compose -f $LTI_DIR/docker-compose.yml exec -T broker bundle exec rake db:apps:update["$APP_NAME","$CALLBACK_URI","$OAUTH_KEY","$OAUTH_SECRET"] \
...
# Docker and Docker Compose are expected to be installed and managed OUTSIDE of
# this script (for example with your own Ansible tasks).  This function only
# verifies that both are available and usable - it never installs, upgrades,
# downgrades or purges any Docker package.
check_docker() {
  need_pkg ca-certificates curl openssl

  if ! command -v docker > /dev/null 2>&1; then
    err "docker is not installed - this script no longer installs Docker, please provision it first (e.g. with your Ansible tasks) and re-run."
  fi

  # Ensuring the docker daemon is reachable (do not restart it, that would kill
  # containers that are already running on this host).
  if ! docker version > /dev/null 2>&1; then
    say "docker is installed but not responding, trying to start the docker service..."
    systemctl start docker.socket docker.service || true
    sleep 5

    docker version > /dev/null 2>&1 || err "docker is installed but the daemon is not running - fix the docker service and re-run."
  fi

  detect_docker_compose

  say "docker is running and using compose command: ${DOCKER_COMPOSE_CMD[*]}"
}

# Detects which Docker Compose implementation is available on the system and
# stores it in DOCKER_COMPOSE_CMD.  Both the Compose v2 plugin ("docker compose")
# and the standalone binary ("docker-compose") are supported.  Nothing is
# installed or removed here.
detect_docker_compose() {
  ...
  if [ -n "$DOCKER_COMPOSE_BIN" ]; then
    # Honour an explicit override, e.g. DOCKER_COMPOSE_BIN="docker-compose".
    read -r -a DOCKER_COMPOSE_CMD <<< "$DOCKER_COMPOSE_BIN"
    "${DOCKER_COMPOSE_CMD[@]}" version > /dev/null 2>&1 || \
      err "DOCKER_COMPOSE_BIN is set to '$DOCKER_COMPOSE_BIN' but that command does not work."
    return 0
  fi
  ...
  if docker compose version > /dev/null 2>&1; then
    ...
    err "no working Docker Compose found - install either the docker compose plugin or the docker-compose binary (e.g. with your Ansible tasks) and re-run."
  fi
}

# Thin wrapper so the rest of the script can call compose without caring about
  ...
  for gl_dir in ~/greenlight $GL3_DIR;do
    if [ -f "$gl_dir"/.env ]; then
      if ! grep ^BIGBLUEBUTTON_ENDPOINT "$gl_dir"/.env | grep -q https; then
        ...
        docker_compose -f "$gl_dir"/docker-compose.yml down
        docker_compose -f "$gl_dir"/docker-compose.yml up -d
      fi
    fi
  done
